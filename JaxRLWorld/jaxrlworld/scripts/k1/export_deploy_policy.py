...
observation normalizer baked in. jaxrlworld's policy is JAX + Equinox, so
...
Run on the training box (needs the sim backend the checkpoint used)::
    ...
    jaxpy -m jaxrlworld.scripts.k1.export_deploy_policy \
...
class DeployActor(nn.Module):
    """TorchScript-able port of the deterministic jaxrlworld actor."""
