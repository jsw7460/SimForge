"""Train the K1 velocity task with the adversarial motion prior on genesis.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="K1 velocity + motion prior on genesis")
    parser.add_argument("--style-weight", type=float, default=None)
    args, _overrides = parser.parse_known_args()

    preset = K1AmpConfig(sim_type="genesis")
