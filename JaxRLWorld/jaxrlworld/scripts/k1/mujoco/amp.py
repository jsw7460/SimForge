"""Train the K1 velocity task with the adversarial motion prior on mujoco.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="K1 velocity + motion prior on mujoco")
    parser.add_argument("--style-weight", type=float, default=None)
    args, _overrides = parser.parse_known_args()

    preset = K1AmpConfig(sim_type="mujoco")
