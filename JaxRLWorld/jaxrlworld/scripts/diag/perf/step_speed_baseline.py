...
it before the change, keep the output, run it again after.
    ...
    python -m jaxrlworld.scripts.diag.perf.step_speed_baseline --num-envs 4096

or one backend, or another preset:

    python -m jaxrlworld.scripts.diag.perf.step_speed_baseline --sim newton --num-envs 4096
    python -m jaxrlworld.scripts.diag.perf.step_speed_baseline --preset yam_lift --num-envs 4096

or with a solver budget the presets do not carry, to find out whether the
budget is what a cross-backend gap is made of:

    python -m jaxrlworld.scripts.diag.perf.step_speed_baseline --preset yam_lift --iterations 10 --ls-iterations 20
...
def run_all(
    ...
    for sim in _SIMS:
        ...
        cmd = [
            sys.executable,
            "-m",
            "jaxrlworld.scripts.diag.perf.step_speed_baseline",
            "--preset",
            preset,
            "--sim",
            sim,
