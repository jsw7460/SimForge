...
Run all three and cross-compare::
    ...
    python -m jaxrlworld.scripts.diag.parity.site_parity_diag --num-envs 4

or one backend::

    python -m jaxrlworld.scripts.diag.parity.site_parity_diag --sim newton --num-envs 4
"""
...
from jaxrlworld.rl.envs.site_frames import sites_from_mjcf
from jaxrlworld.rl.runners import BaseRunner
from jaxrlworld.rl.utils.quat_utils import quat_inv_wxyz, quat_mul_wxyz
...
def run_all(num_envs: int) -> int:
    ...
    for sim in _SIMS:
        ...
        cmd = [
            sys.executable,
            "-m",
            "jaxrlworld.scripts.diag.parity.site_parity_diag",
            "--sim",
            sim,
            "--result-json",
            str(result_path),
