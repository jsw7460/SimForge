...
mirroring IsaacLab's ``TerrainImporter`` / mjlab's ``TerrainEntity``:
  ...
  * ``jaxrlworld/rl/terrains/importer.py``                — base class
  * ``jaxrlworld/rl/envs/managers/<sim>/terrain_importer.py``
...
Usage::
    ...
    python -m jaxrlworld.scripts.diag.gates.check_terrain_importer_refactor
    python -m jaxrlworld.scripts.diag.gates.check_terrain_importer_refactor --sim newton
    python -m jaxrlworld.scripts.diag.gates.check_terrain_importer_refactor --sim genesis --terrain rough
    python -m jaxrlworld.scripts.diag.gates.check_terrain_importer_refactor \\
...
_PRESETS: dict[str, dict] = {
    "go2": {
        "module": "jaxrlworld.rl.configs.presets.go2.base",
        "class_name": "Go2FlatConfig",
        "supports_rough": True,
        "urdf_body_name": "base",
    },
    "g1_29dof": {
        "module": "jaxrlworld.rl.configs.presets.g1_29dof.base",
...
def _check_termination_wiring(env, sim: str, use_rough: bool, lg: _Logger, combo: str) -> bool:
    ...
    from jaxrlworld.rl.envs.mdp.terminations.common.terminations import terrain_out_of_bounds
...
def _check_viser_bridge_geometry(env, sim: str, use_rough: bool, lg: _Logger, combo: str) -> bool:
    ...
    try:
        if sim == "genesis":
            from jaxrlworld.rl.vis.viser.bridges import GenesisBridge as Bridge
        else:
            from jaxrlworld.rl.vis.viser.bridges import MujocoBridge as Bridge
...
def _spawn_subprocess(
    ...
    *,
    ...
    cmd = [
        sys.executable,
        "-m",
        "jaxrlworld.scripts.diag.gates.check_terrain_importer_refactor",
        "--sim",
        sim,
        "--terrain",
        terrain,
