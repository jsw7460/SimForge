...
Run (server):
    python -m jaxrlworld.scripts.diag.k1.k1_g1recipe_reward_parity_diag
"""
...
_MODULE = "jaxrlworld.scripts.diag.k1.k1_g1recipe_reward_parity_diag"
...
def _term_map(sim: str, task: str) -> dict[str, tuple[str, str]]:
    ...
    k1mod = "jaxrlworld.rl.envs.mdp.rewards.k1_locomotion"
    if task == "joystick":
    ...
    mod = f"jaxrlworld.rl.envs.mdp.rewards.{sim}.mjlab_rewards"
    return {
...
def run_cell(sim: str, num_envs: int, settle_steps: int, seed: int, task: str) -> dict:
    ...
    if fix and sim == "genesis":
        ...
        from jaxrlworld.rl.envs.genesis.robot_state_writer import GenesisRobotStateWriter as _W
    ...
    if fix:
        ...
        mj_mod = importlib.import_module(f"jaxrlworld.rl.envs.mdp.rewards.{sim}.mjlab_rewards")
    ...
    if task == "joystick":
        from jaxrlworld.rl.configs.presets.k1_joystick.base import K1JoystickConfig as _Cfg
    else:
        from jaxrlworld.rl.configs.presets.k1_joystick.g1_recipe import K1G1RecipeConfig as _Cfg
