...
``jaxrlworld.rl.envs.mdp.events.dr._model_fields.collect_expand_fields``.

Run (GPU box)::

    jaxpy -m jaxrlworld.scripts.diag.dr.dr_model_fields_diag
    jaxpy -m jaxrlworld.scripts.diag.dr.dr_model_fields_diag --static-only  # no env build
...
def _retired_collect(event_cfg) -> tuple[str, ...]:
    ...
    from jaxrlworld.rl.envs.mdp.events.dr import unified as _unified
...
def _all_dr_config():
    ...
    from jaxrlworld.rl.envs.mdp.events.dr import unified as u
...
def check_c4() -> bool | None:
    from jaxrlworld.rl.envs.mdp.events.dr._model_fields import DERIVED_FIELDS, RecomputeLevel
