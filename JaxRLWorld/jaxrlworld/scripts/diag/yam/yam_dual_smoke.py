...
Run all three and cross-compare::
    ...
    python -m jaxrlworld.scripts.diag.yam.yam_dual_smoke --num-envs 4

or one backend::

    python -m jaxrlworld.scripts.diag.yam.yam_dual_smoke --sim mujoco --num-envs 4
"""
...
from jaxrlworld.rl.configs.presets.yam_dual.base import RIGHT_ROBOT, YamDualArmConfig
from jaxrlworld.rl.envs.mdp.observations.common import proprioception as obs_mdp
...
def run_all(num_envs: int) -> int:
    ...
    for sim in _SIMS:
        ...
        cmd = [
            sys.executable,
            "-m",
            "jaxrlworld.scripts.diag.yam.yam_dual_smoke",
            "--sim",
            sim,
            "--result-json",
            str(result_path),
