"""TD3 on Gymnasium ``Swimmer-v5``.

Mirrors ``td3/halfcheetah.py`` (manual ``gym.make`` + FlattenObservation
+ SyncVectorEnv -> GymnasiumEnv -> OffPolicyRunner), pointed at the real
gymnasium Swimmer-v5 task.

``ACTION_REPEAT=1`` is the standard gym Swimmer-v5 setting; try 4 if learning
stalls.

Run (GPU box):
    python jaxrlworld/scripts/benchmark/td3/swimmer.py
"""
...
from jaxrlworld.rl.configs import TD3PolicyConfig
from jaxrlworld.rl.configs.algorithms import TD3Config
...
def main():
    ...
    cfgs_for_run.runner.run_name = "Swimmer_TD3"
