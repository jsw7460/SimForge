...
#
# Run from anywhere:  bash JaxRLWorld/jaxrlworld/scripts/benchmark/sb3_compare/ppo_halfcheetah.bash
...
for SEED in 0 1 2; do
    ( START=$(date +%s); BENCH_SEED=$SEED \
        ...
        echo $(($(date +%s) - START)) > "$LOG_DIR/sb3_ppo_halfcheetah_s$SEED.sec" ) &
    PIDS+=($!)
    ...
    ( START=$(date +%s); BENCH_SEED=$SEED XLA_PYTHON_CLIENT_PREALLOCATE=false \
        ...
        echo $(($(date +%s) - START)) > "$LOG_DIR/jrw_ppo_halfcheetah_s$SEED.sec" ) &
    PIDS+=($!)
...
for SEED in 0 1 2; do
    for FW in sb3 jrw; do
        SECS=$(cat "$LOG_DIR/${FW}_ppo_halfcheetah_s$SEED.sec" 2>/dev/null || echo "?")
        if [ "$FW" = jrw ]; then
            RET=$(grep -a "Mean Return" "$LOG_DIR/jrw_ppo_halfcheetah_s$SEED.log" | tail -1 | awk '{print $NF}')
        else
            RET=$(grep -a "ep_rew_mean" "$LOG_DIR/sb3_ppo_halfcheetah_s$SEED.log" | tail -1 | awk '{print $(NF-1)}')
        fi
