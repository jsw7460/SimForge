...
from jaxrlworld.rl.utils.console import GREEN, RED, RESET, YELLOW
...
class DatasetCheckpointHandler:
    ...
    def save_dataset_checkpoint(
        runner: "BaseRunner", dataset: DynamicsDataset, path: str, include_policy: bool = True
    ) -> None:
        """
        ...
        if include_policy:
            checkpoint["policy_state"] = {name: module.state_dict() for name, module in runner.training_modules.items()}
    ...
    def load_dataset_checkpoint(
        path: str, load_policy: bool = False, runner: Optional["BaseRunner"] = None
