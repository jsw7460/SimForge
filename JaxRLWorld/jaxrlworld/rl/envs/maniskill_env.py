...
JaxRLWorld runner contract -- the same one ``jaxrlworld.rl.envs.gymnasium_env.
...
``jaxrlworld/scripts/diag/check_maniskill_adapter.py``):
...
``jaxrlworld.rl.runners.base_runner._create_env_from_config`` so the runner can
