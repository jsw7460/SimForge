    ...
    def __init__(
        ...
        if "body_names" not in data.files:
            raise ValueError(
                ...
                "Regenerate via jaxrlworld.tools.motion.csv_to_npz."
            )
        ...
        if joint_names_cfg is not None:
            if "joint_names" not in data.files:
                raise ValueError(
                    ...
                    "the NPZ via jaxrlworld.tools.motion.csv_to_npz."
                )
...
class MotionCommandCfg(CommandTermCfg):
    ...
    """NPZ paths to track (produced by ``jaxrlworld.tools.motion.csv_to_npz``
