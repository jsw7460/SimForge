...
from jaxrlworld.rl.envs.mdp.commands.motion import MotionCommand
from jaxrlworld.rl.utils.quat_utils import quat_error_magnitude_wxyz
...
def motion_global_anchor_position_error_exp(
    ...
    err = torch.sum(
        torch.square(cmd.anchor_pos_w - cmd.robot_anchor_pos_w),
        dim=-1,
    )
...
def motion_relative_body_position_error_exp(
    ...
    err = torch.sum(
        torch.square(cmd.body_pos_relative_w[:, idx] - cmd.robot_body_pos_w[:, idx]),
        dim=-1,
    )
...
def motion_relative_body_orientation_error_exp(
    ...
    err = (
        quat_error_magnitude_wxyz(
            cmd.body_quat_relative_w[:, idx],
            cmd.robot_body_quat_w[:, idx],
        )
        ** 2
    )
    return torch.exp(-err.mean(-1) / std**2)
...
def motion_global_body_linear_velocity_error_exp(
    ...
    idx = _get_body_indexes(cmd, body_names)
    err = torch.sum(
        torch.square(cmd.body_lin_vel_w[:, idx] - cmd.robot_body_lin_vel_w[:, idx]),
        dim=-1,
    )
    return torch.exp(-err.mean(-1) / std**2)
...
def motion_global_body_angular_velocity_error_exp(
    ...
    idx = _get_body_indexes(cmd, body_names)
    err = torch.sum(
        torch.square(cmd.body_ang_vel_w[:, idx] - cmd.robot_body_ang_vel_w[:, idx]),
        dim=-1,
    )
    return torch.exp(-err.mean(-1) / std**2)
