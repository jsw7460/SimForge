...
Actuator types are defined in :mod:`jaxrlworld.rl.actuators.actuator_cfg`.
The actuator class determines the control mode:

- :class:`~jaxrlworld.rl.actuators.ImplicitActuatorCfg` → simulator PD
...
if TYPE_CHECKING:
    from jaxrlworld.rl.actuators.actuator_cfg import ActuatorBaseCfg
...
class ArticulationCfg:
    ...
    Attributes:
        actuators: Tuple of actuator config objects (from
            :mod:`jaxrlworld.rl.actuators`), one per actuator group.
            The actuator **type** determines the control mode:
            :class:`~jaxrlworld.rl.actuators.ImplicitActuatorCfg` uses
...
class RigidObjectCfg(EntityCfg):
    ...
    :class:`~jaxrlworld.rl.envs.robot_data.RigidObjectData` (root + body reads, no
    joint accessors).
