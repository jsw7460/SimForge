...
def track_lin_vel(env: World, std: float = 0.25, entity_name: str = "robot") -> torch.Tensor:
    ...
    actual = env.get_robot_data(entity_name).root_link_lin_vel_b[:, :2]
    error = torch.sum(torch.square(target - actual), dim=1)
    return torch.exp(-error / std ** 2)


def track_ang_vel(env: World, std: float = 0.25, entity_name: str = "robot") -> torch.Tensor:
    """Reward for tracking commanded angular velocity (yaw).

    Returns:
        Tensor of shape (num_envs,).
    """
    actual_yaw_rate = env.get_robot_data(entity_name).root_link_ang_vel_b[:, 2]
...
def flat_orientation(env: World, entity_name: str = "robot") -> torch.Tensor:
