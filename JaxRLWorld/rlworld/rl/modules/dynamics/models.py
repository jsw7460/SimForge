...
class DynamicsDecoder(nn.Module):
    ...
    def __init__(self, feature_dim: int, action_dim: int, output_dim: int, hidden_dims: List[int] = [256, 128]):
        super().__init__()
...
class MLPDynamics(DynamicsModel):
    ...
    def __init__(self, input_dim: int, action_dim: int, output_dim: int, hidden_dims: List[int] = [512, 512, 256]):
        super().__init__()
        ...
        for hidden_dim in hidden_dims:
            layers.extend([nn.Linear(in_dim, hidden_dim), nn.ReLU(), nn.Dropout(0.2), nn.LayerNorm(hidden_dim)])
            in_dim = hidden_dim
...
def create_models(
    ...
    if encoder_type == "hybrid":
        ...
        encoder = ABAEncoder(kinematic_tree=kinematic_tree, obs_dim=input_dim, **encoder_config).to(device)
