"""Real-time reward/metric plotting for Viser viewer.

Adapted from Mjlab's ViserTermPlotter with minimal changes.
"""
...
class ViserTermPlotter:
    ...
    def _create_plot(self, state: _TermState) -> None:
        """Lazily create a single-term plot."""
        h = state.history
        hist_len = len(h)
        if hist_len > 0:
    ...
    def update(self, terms: dict[str, float]) -> None:
        """Push new data and refresh visible plots.

        Args:
            terms: Mapping of term name to scalar value for the selected env.
        """
        any_enabled = False
        for tname, val in terms.items():
