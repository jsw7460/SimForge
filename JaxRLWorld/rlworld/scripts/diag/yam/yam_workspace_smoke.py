...
Run all three and cross-compare::
    ...
    python -m rlworld.scripts.diag.yam.yam_workspace_smoke --num-envs 4

or one backend::

    python -m rlworld.scripts.diag.yam.yam_workspace_smoke --sim mujoco --num-envs 4
"""
...
def run_all(num_envs: int, settle: int) -> int:
    ...
    for sim in _SIMS:
        ...
        cmd = [
            sys.executable,
            "-m",
            "rlworld.scripts.diag.yam.yam_workspace_smoke",
            "--sim",
            sim,
            "--result-json",
            str(result_path),
