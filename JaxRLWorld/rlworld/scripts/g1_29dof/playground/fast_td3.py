    ...
    for global_step in range(TOTAL_TIMESTEPS):
        ...
        if global_step >= LEARNING_STARTS:
            ...
            if global_step % 100 == 0:
                ...
                m = metrics.to_full_dict()
                log_data = {
                    "speed": speed,
                    "env_reward_mean": float(rewards.mean()),
                    "critic/loss": m.get("critic/loss", 0),
                    "critic/q1_mean": m.get("critic/q1_mean", 0),
                    "critic/q_target_mean": m.get("critic/q_target_mean", 0),
                    "actor/loss": m.get("actor/loss", 0),
                    "batch/reward_mean": m.get("batch/reward_mean", 0),
                }
                print(
                    f"[{global_step:>6d}/{TOTAL_TIMESTEPS}] "
                    f"speed={speed:.0f} sps | "
                    f"env_rew={float(rewards.mean()):.3f} | "
                    f"critic_loss={log_data['critic/loss']:.4f} | "
                    f"q1={log_data['critic/q1_mean']:.2f}"
                )
                if USE_WANDB:
