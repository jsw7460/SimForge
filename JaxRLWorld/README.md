...
(via [mjlab][mjlab]) — using a single sim-agnostic API.
...
- **PPO is the default for all locomotion tasks** across the three
  ...
  small subset of Gymnasium-based benchmark suite (see `scripts/benchmark/`), with
  FastTD3 additionally validated on [mujoco_playground][mjpg].
...
│   ├── envs/            # Sim-agnostic env layer + per-sim adapters
│   ├── modules/         # Network architectures (MLP, ST-Transformer)
│   ├── runners/         # On-policy / Off-policy / Model-based runners
...
JaxRLWorld is released under the MIT License. See [LICENSE](LICENSE) for
details. Note that the upstream projects we build on carry their own
licenses; please consult each one when redistributing.
